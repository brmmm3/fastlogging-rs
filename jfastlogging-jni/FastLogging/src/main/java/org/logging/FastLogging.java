...
public class FastLogging {
	...
	static {
		System.loadLibrary("jfastlogging_jni");
	}
	...
	public static native long loggingNew(int level, String domain, Long configs, Long extConfig, String configPath);
	...
	private static native int loggingAddWriter(long instance_ptr, long writer_ptr);
	...
	static public class Logging {
		...
		public Logging(int level) {
			instance_ptr = loggingNew(level, "root", null, null, null);
		...
		public Logging(String path) {
			instance_ptr = loggingNew(NOTSET, null, null, null, path);
		}
		...
		public void setLevel(int wid, int level) {
			loggingSetLevel(instance_ptr, wid, level);
			instance_level = level;
		}
